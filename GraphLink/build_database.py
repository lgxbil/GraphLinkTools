...
class APIDocEmbedder:
    ...
    def _setup_collection(self):
        ...
        if not self.client.has_collection(self.collection_name):
            ...
            schema.add_field(field_name="required", datatype=DataType.INT64,default_value=2)
    ...
    def prepare_data(self, api_folder_path):
        ...
        for i, file_path in enumerate(tqdm(file_paths, desc="Processing API files",total=len(file_paths))):
            ...
            if response_desc_list not in ("", [], {}, None):
                ...
                #存入数据库里 这里required虽然是默认值 但是不填会报错
                data1 = []
                for i, res in enumerate(response_desc_list):
                    data1.append({
                        "type": "response",
            ...
            if notebook.enhance_params_desc["enhanced_parameters"] not in ("", [], {}):
                ...
                print(params_embeddings.shape)
                data2 = []
                for i, (name, value) in enumerate(notebook.enhance_params_desc["enhanced_parameters"].items()):
                    data2.append({
                        "type": "params",
                        "path_name": "",
                        "required": 1 if notebook.params["properties"][name]["required"] else 0,
                        "api_name":api_name,
                        "native_name": name,
                        "desc":value,
    ...
    def insert_one(self,):
        res_strlist = {
            "$.legs[].fromId":"Unique identifier for the departure airport/location.",
            "$.legs[].toId":"Unique identifier for the arrival airport/location.",
            "$.legs[].date": "Date of departure for the flight.",
        }
        param_strlist = []
        for i, (name, value) in enumerate(res_strlist.items()):
            desc = f"{name}:{value}"
            param_strlist.append(desc)
    ...
    # 直接返回结果 不带任何过滤条件的
    def search_by_vector(self, query_vector, topk=15):
        res = self.client.search(
            collection_name=self.collection_name,
            query_vector=query_vector,
            limit=topk,
            output_fields=["id", "type", "path_name", "required", "api_name", "native_name", "desc", "desc_embed"]
        )
    ...
    def query_by_api(self, api_name):
        ...
        return res_class
    #根据响应搜索参数
    def search_api_params(self, query_vector, topk=15):
        pass
    #根据一个api的参数搜索响应
    def search_api_response(self, api_name:None, query_vector:List, topk=15, ):
        ...
        return res
    #删除数据
    def delete_api_params(self, api_name=None):
        """删除指定API的参数和响应"""
        filter_params = f"api_name == 'Search_Flights_Multi_Stops' and type == 'params' and native_name == 'legs'"
        res = self.client.delete(collection_name=self.collection_name, filter=filter_params)
        print(res)
    #查询有多少条数据
    def get_entity_count(self) -> int:
        """返回当前集合中的实体数量"""
        stats = self.client.get_collection_stats(collection_name=self.collection_name)
        return stats.get("row_count", 0)  # 从统计信息中提取行数
    
if __name__ == "__main__":
    #两个向量数据库名字是all_minv3_api_docs 和 api_docs
    ...
    # api_folder_path = "/home/snrobot/lin/GraphLinkTools/Tools(1)/ApiInfo_update(1)"
    # apiembeder.prepare_data(api_folder_path)
    
    # c = apiembeder.get_entity_count()
    # # apiembeder.insert_one()
    # print(c)
    res = apiembeder.query_by_api("Search_Flights_Multi_Stops")
    for item in res["params"]:
        print(item["native_name"] + ": " + item["desc"])
    print(type(res["params"][0]["desc_embed"]))
    res1 = apiembeder.search_api_response(api_name=None, query_vector= [res["params"][6]["desc_embed"]], topk=70)
    # print(res1[1])
    res_list = []
    for item in res1[0]:
        res = {}
        res[item["api_name"]] = str(item["path_name"] + ":" +item["desc"])
        res_list.append(res)
