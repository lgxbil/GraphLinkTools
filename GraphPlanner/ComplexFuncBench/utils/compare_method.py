...
from ComplexFuncBench.models.gpt import GPTModel
from ComplexFuncBench.models.deepseek import DeepSeekModel
from ComplexFuncBench.prompts.compare import system_prompt, user_prompt
from ComplexFuncBench.utils.logger import Logger

class CompareFCBase:
    def __init__(self, args, logger) -> None:
        self.embedding = FlagModel('/home/snrobot/lin/GraphLinkTools/GraphPlanner/ComplexFuncBench/baai', 
        ...
        self.model = DeepSeekModel(**params)
        self.logger = logger
        self.error_message = []
        self.exact_match_dict = load_json("/home/snrobot/lin/GraphLinkTools/GraphPlanner/ComplexFuncBench/utils/exact_match_values.json")
...
class CompareFC(CompareFCBase):
        ...
        for k, v in golden_call['arguments'].items():
            if k in param_list:
                if k not in pred_call['arguments']:
                    ...
                    return False, {"error_type": "value_error", "content": f"Parameter {k} value '{pred_call['arguments'][k]}' is not correct in prediction."}
    ...
    def get_error_message(self, pred_call, golden_call):
        ...
        for k, v in golden_call['arguments'].items():
            ...
            if v != pred_call['arguments'][k]:
                return {"error_type": "value_error", "content": f"Parameter {k} value '{pred_call['arguments'][k]}' do not equal to golden."}
    ...
    def compare_turn_prediction(self, functions, history, predict, golden, golden_obs):
        ...
        format_error = {}
        success_map, success_matched, success_matched_func = {}, [], []
        for match_item in match_list:
            ...
            if message == True:
                ...
                if is_match:
                    ...
                    success_matched_func.append(str(match_item['golden_call']))
                else:
                    if single_message:
        ...
        return self.error_message, success_map, success_matched, format_error, success_matched_func